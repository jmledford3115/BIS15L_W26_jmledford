...
```{r}
z=10 #this will work, but will create problems
```
...
```{r}
mean(c(venom_GT, chiron, veyron))#to use a vector
```
...
```{r}
my_integer <- as.integer(my_numeric)#important when we work with plots
```
...
```{r}
new_vector <- c(7, 6.2, 5, 9, NA, 4, 9.8, 7, 3, 2) #create a new vector
```
...
```{r}
mean(new_vector, na.rm = T) #removes NA values
```